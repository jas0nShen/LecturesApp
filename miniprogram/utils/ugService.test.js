...
test('CUHK Theology exposes the current 44-course BA Major and Area list as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'THEON');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-THEON-15-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2025');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.match(byCode.THEO3255.requirementGroups[0], /repeatable in different terms/);
  assert(ugService.listMajorCourses(programme.id, major.id, { keyword: 'Biblical Hebrew' }).some((course) => course.courseCode === 'THEO3217'));
});

test('CUHK Social Work exposes the current 32-course Study Scheme list as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'SOWKN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-SOWKN-81-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2025');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(programme.sourceStatus, 'course_codes_available');
  assert.equal(profile.totalCreditRequired, 0);
  assert.equal(profile.codedCourseCount, 32);
  ...
  assert.equal(byCode.SOWK2050.courseType, 'major_elective');
...
test('CUHK Financial Technology exposes the official 85-course scope as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'FTECN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-FTECN-46-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(courses.filter((course) => course.courseType === 'capstone').length, 2);
  assert.equal(courses.filter((course) => course.courseType === 'internship').length, 2);
  assert.equal(byCode.ENGG1110.titleEn, 'Problem Solving By Programming');
  assert.equal(byCode.ACCT2111.titleEn, 'Introductory Financial Accounting');
  assert.equal(byCode.FTEC2001.credits, 2);
  assert.equal(byCode.FTEC2602.credits, 1);
...
test('CUHK Computational Data Science exposes the official 33-code required scope as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'CDASN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-CDASN-40-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(courses.filter((course) => course.courseType === 'capstone').length, 2);
  assert.equal(byCode.ENGG1111.credits, 0);
  assert.equal(byCode.STAT3010.titleEn, 'Optimization for Statistics and Data Science');
...
test('CUHK Information Engineering exposes the current 135-course named scope as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'IERGN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-IERGN-47-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(programme.sourceStatus, 'course_codes_available');
  assert.equal(profile.totalCreditRequired, 0);
  assert.equal(profile.codedCourseCount, 135);
  ...
  assert.match(byCode.IERG3840.requirementGroups[0], /choose IERG3840 or IERG3842/);
  assert.match(byCode.IERG4998.requirementGroups[0], /Research Component Courses/);
  assert.match(byCode.CSCI4180.requirementGroups[0], /optional 12-unit Stream pool/);
  assert.equal(
    byCode.IERG5360.titleEn,
    'Program Representation, Modeling and Understanding for Software Security'
  );
});

test('CUHK Materials Science and Engineering exposes the current 58-code department list as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'MASEN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-MASEN-48-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.match(byCode.ENGG1110.requirementGroups[0], /slash-separated ENGG\/ESTR codes are alternatives/);
  assert.match(byCode.CSCI1120.titleEn, /Introduction to Computing Using C\+\+/);
  assert.equal(
...
test('CUHK Quantitative Finance and Risk Management Science exposes the current 122-code scope as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'QFRMN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-QFRMN-27-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(programme.sourceStatus, 'course_codes_available');
  assert.equal(profile.totalCreditRequired, 0);
  assert.equal(profile.codedCourseCount, 122);
  ...
  assert.match(byCode.MATH1530.requirementGroups[0], /Placement Test/);
  assert.match(byCode.ACCT4214.requirementGroups[0], /ACCT4212, ACCT4213 or ACCT4214/);
  assert.match(byCode.FINA3310.requirementGroups[0], /no more than six 1-unit courses/);
  assert.match(byCode.RMSC4005.requirementGroups[0], /Risk Management Science/);
...
test('CUHK Sociology exposes the 2025-26 62-course Major pool as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'SOCIN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-SOCIN-83-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert(ugService.listMajorCourses(programme.id, major.id, { keyword: 'Video Analysis' }).some((course) => course.courseCode === 'SOCI3235'));
});

test('CUHK Religious Studies exposes all current Religious Studies Area courses as browse-only', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'RELSN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-RELSN-14-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2025');
  const courses = ugService.listMajorCourses(programme.id, major.id);
  const byCode = Object.fromEntries(courses.map((course) => [course.courseCode, course]));
  ...
  assert.equal(byCode.CURE1110.credits, 3);
  ...
  assert.equal(byCode.CURE1009, undefined);
  assert.equal(byCode.PSYC3720, undefined);
  assert(ugService.listMajorCourses(programme.id, major.id, { keyword: 'Religious Imagination' }).some((course) => course.courseCode === 'CURE3130'));
});

test('CUHK Public History exposes only the official ten-course highlight list', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'HISTN-PUH');
  const major = ugService.listMajors(programme.id).find((item) => item.nameEn === 'Public History');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);

  assert.equal(programme.sourceStatus, 'course_codes_available');
  assert.equal(profile.codedCourseCount, 10);
  assert.equal(courses.length, 10);
  assert(courses.every((course) => course.credits === 0));
  assert(courses.some((course) => course.courseCode === 'HIST1700' && course.courseType === 'core'));
  assert(courses.some((course) => course.courseCode === 'HIST4702' && course.titleEn === 'Digital History'));
  assert(courses.some((course) => course.courseCode === 'HIST4180RH' && course.requirementGroups.some((group) => group.includes('stable base code not published'))));
  assert(ugService.listMajorCourses(programme.id, major.id, { keyword: 'Oral History' }).some((course) => course.courseCode === 'HIST2010'));
});

test('CUHK Early Childhood Education exposes the complete 55-unit Major course structure', () => {
  const cuhk = ugService.listUniversities().find((item) => item.code === 'CUHK');
  const programmes = ugService.listProgrammes({ universityId: cuhk.id, degreeLevel: 'undergraduate' });
  const programme = programmes.find((item) => item.code === 'CHEDN');
  const major = ugService.listMajors(programme.id).find((item) => item.id === 'CUHK-UG-CHEDN-30-M1');
  const profile = ugService.getMajorProfile(programme.id, major.id, '2026');
  const courses = ugService.listMajorCourses(programme.id, major.id);

  assert.equal(programme.sourceStatus, 'course_codes_available');
  assert.equal(profile.codedCourseCount, 31);
  assert.equal(courses.length, 31);
  assert.equal(courses.filter((course) => course.courseType !== 'major_elective').reduce((sum, course) => sum + course.credits, 0), 47);
  assert.equal(courses.filter((course) => course.courseType === 'major_elective').length, 15);
  assert(courses.some((course) => course.courseCode === 'CHED4540' && course.courseType === 'capstone' && course.credits === 4));
  assert(courses.some((course) => course.courseCode === 'CHED4550' && course.courseType === 'internship' && course.credits === 3));
  assert(ugService.listMajorCourses(programme.id, major.id, { keyword: 'Music Therapy' }).some((course) => course.courseCode === 'MUSC2545'));
});
...
test('UG school coverage summarizes imported source data for the status page', () => {
  ...
  assert.equal(hku.majorCount, 145);
  assert.equal(hku.programmeWithCoursesCount, 92);
  assert.equal(hku.pendingProgrammeCount, 44);
  assert.equal(hku.coveragePercent, 68);
  assert.equal(hku.codedCourseCount, 6033);
