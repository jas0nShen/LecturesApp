...
test('UG catalogue summarizes current undergraduate seed data', () => {
  ...
  assert.equal(summary.codedCourseCount, 5271);
  assert.equal(summary.programmeWithCoursesCount, 73);
  assert.equal(summary.pendingProgrammeCount, 371);
...
test('UG pending programme collection text is copy-ready for data sourcing', () => {
  ...
  assert.equal(allPending.length, 41);
  ...
  assert.match(text, /待补 Programme：41/);
...
test('UG per-school coverage stays visible for setup validation', () => {
  ...
  assert.deepEqual(coverage, {
    ...
    POLYU: { programmeCount: 46, majorCount: 110, codedCourseCount: 821 },
...
test('UG school coverage summarizes imported source data for the status page', () => {
  ...
  assert.equal(polyu.programmeWithCoursesCount, 5);
  assert.equal(polyu.pendingProgrammeCount, 41);
  assert.equal(polyu.coveragePercent, 11);
  assert.equal(polyu.codedCourseCount, 821);
