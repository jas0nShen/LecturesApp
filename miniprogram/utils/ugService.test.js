...
test('UG catalogue summarizes current undergraduate seed data', () => {
  ...
  assert.equal(summary.codedCourseCount, 5591);
  assert.equal(summary.programmeWithCoursesCount, 76);
  assert.equal(summary.pendingProgrammeCount, 368);
...
test('UG pending programme collection text is copy-ready for data sourcing', () => {
  ...
  assert.equal(allPending.length, 38);
  ...
  assert.match(text, /待补 Programme：38/);
...
test('UG per-school coverage stays visible for setup validation', () => {
  ...
  assert.deepEqual(coverage, {
    ...
    POLYU: { programmeCount: 46, majorCount: 110, codedCourseCount: 1141 },
...
test('UG school coverage summarizes imported source data for the status page', () => {
  ...
  assert.equal(polyu.programmeWithCoursesCount, 8);
  assert.equal(polyu.pendingProgrammeCount, 38);
  assert.equal(polyu.coveragePercent, 17);
  assert.equal(polyu.codedCourseCount, 1141);
