...
test('UG catalogue summarizes current undergraduate seed data', () => {
  ...
  assert.equal(summary.codedCourseCount, 6797);
  assert.equal(summary.programmeWithCoursesCount, 91);
  assert.equal(summary.pendingProgrammeCount, 353);
...
test('UG pending programme collection text is copy-ready for data sourcing', () => {
  ...
  assert.equal(allPending.length, 23);
  ...
  assert.match(text, /待补 Programme：23/);
...
test('UG per-school coverage stays visible for setup validation', () => {
  ...
  assert.deepEqual(coverage, {
    ...
    POLYU: { programmeCount: 46, majorCount: 110, codedCourseCount: 2347 },
...
test('UG school coverage summarizes imported source data for the status page', () => {
  ...
  assert.equal(polyu.programmeWithCoursesCount, 23);
  assert.equal(polyu.pendingProgrammeCount, 23);
  assert.equal(polyu.coveragePercent, 50);
  assert.equal(polyu.codedCourseCount, 2347);
