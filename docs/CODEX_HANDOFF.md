...
当前仓库发布资料已统一为 `1.0.16`（`package.json`、运行时 `releaseInfo`、`docs/MVP_SPEC.md`、`docs/RELEASE_CHECKLIST.md`、`docs/REVIEW_SUBMISSION.md`）。CUHK `HTMGB Hospitality and Real Estate` 发布提交 `3dab741` 已推送到 `origin/main`；本地提交 `a5b7782` 已加入 `BEHMN Biotechnology, Entrepreneurship and Healthcare Management`，尚未推送。本次同日继续加入 `COMMN Journalism and Communication`；按用户指定的发布日递增规则，2026-08-10 的提交继续使用 `1.0.16`，不回退至 `1.0.14`。当前没有可验证的微信 `1.0.15` 或 `1.0.16` 上传成功记录，微信最后明确确认的开发版仍为 `1.0.14`、对应课程提交 `ba1e028`；没有提交审核或发布正式版。
...
- 当前工作区 UG：8 所学校、444 个 Programme、686 个 Major/Track、Programme/Major-local 带代码课程共 20,805 条；315 个 Programme 已开放课程代码。
...
- 当前工作区完整检查：1018/1018 测试通过、`ready=true`、20,805 条 UG coded courses、315 个已开放 Programme、主包预估 1,845,684 bytes、总估算包体 23,947,290 bytes，所有 23 个分包低于 2 MB，敏感 API 为 0。
...
- UG 来源覆盖报告仍有 129 个缺口：35 个 index-only、25 个 reviewed-no-codes、69 个 no-source。CUHK 仍有 33 个缺口，其中 3 个 reviewed-no-codes、30 个 no-source。下一批继续从官方课程手册或部门课程页取证，不从 Programme 简介反推代码或规则。
- 多个已核验 TPG Programme 仍标记 `manual_review_required`，表示课程池可发布，但复杂路径/跨组最低要求不能由当前规则引擎自动证明。

### 发布与验收缺口

- 仓库发布资料已更新为 `1.0.16`；本地提交 `a5b7782` 包含 BEHMN 批次，本次工作区继续加入 COMMN 批次，`origin/main` 仍为 HTMGB 发布提交 `3dab741`。当前没有可验证的微信 `1.0.15` 或 `1.0.16` 上传成功记录，微信最后明确确认的 `1.0.14` 开发版仍对应课程提交 `ba1e028`；本地提交尚未推送或上传。上传不等于已提交审核，当前仍未提交审核或发布正式版。
...
HTMGB source supplement、builder、专项测试、重新生成的 CUHK 双分包、catalogue/shards/runtime tests 及 `1.0.15` 发布资料已由提交 `3dab741` 推送到 `origin/main`。本地提交 `a5b7782` 已加入 BEHMN 与 `1.0.16` 发布资料；本次同日提交继续加入 COMMN source supplement、builder、专项测试、重新生成的 CUHK 双分包、catalogue/shards/runtime tests 与更新后的发布资料。当前没有可验证的微信 `1.0.15` 或 `1.0.16` 上传成功记录；微信最后明确确认的 `1.0.14` 开发版仍覆盖至课程提交 `ba1e028`。提交后工作区应只保留用户本地诊断产物：
...
2026-08-10 在当前 BEHMN、COMMN、`1.0.16` 发布资料与 CUHK 双分包工作区重新运行：
...
npm run check:ship: PASS
  ...
  UG supplement validation: PASS (369 supplements)
  directory audit: PASS, blockingErrors=[]
  server --check: PASS
  Node tests: PASS, 1018/1018
  check:release: PASS, ready=true

git diff --check: PASS
npm run status:tpg-courses -- --school=HKU: PASS (448/448 reviewed, 0 unreviewed)
npm run status:ug-sources -- --missing-only --missing-summary --missing-limit 0: PASS (129 Programme-level source gaps: 35 index-only, 25 reviewed-no-codes, 69 no-source)
npm run status:ug-sources -- --school cuhk --missing-only --missing-summary --missing-limit 0: PASS (33 CUHK gaps: 3 reviewed-no-codes, 30 no-source)
```

发布指标：

- 当前代码主包预估：1,845,684 bytes。
- 当前代码总估算包体：23,947,290 bytes，201 个上传文件。
- 最新上传 1.0.14 的 CLI 实际包：18,676,360 bytes；实际主包 1,485,532 bytes。
- 当前 23 个 UG/TPG 分包均低于 2 MB；`ug-data-cuhk-a` 为 1,259,449 bytes，`ug-data-cuhk-b` 为 1,306,371 bytes，最大分包为 `ug-data-hku-a` 1,869,435 bytes。
- 页面：17；敏感 API：0。
...
- 微信已通过官方 CLI 用课程提交 `ba1e028` 上传 `1.0.14`；没有可验证的 `1.0.15` 或 `1.0.16` 上传成功记录。HTMGB 发布提交 `3dab741` 已推送；本地 BEHMN 提交 `a5b7782` 与本次 COMMN 改动尚未推送或上传。这些状态均不等于已提审或已发布。
