...
Ship and maintain version 1.0.3 of a local-first WeChat mini program for Hong Kong university students. The product lets users select an undergraduate or taught-postgraduate Programme, browse verified offline course data, keep local favourites/completion records, and use conservative planning/audit views without pretending that unreviewed rules are official graduation decisions.
...
Trial/release builds read packaged data and local storage. The development build may use the zero-dependency local Node API, but release must not call a user's `localhost`. This keeps version 1.0.3 deployable without cloud infrastructure and matches the current privacy promise.
