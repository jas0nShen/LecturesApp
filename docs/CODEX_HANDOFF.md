...
最后核验时间：2026-07-21（Asia/Shanghai）
...
当前仓库发布资料已统一为 `1.0.9`（`package.json`、运行时 `releaseInfo`、`docs/MVP_SPEC.md`、`docs/RELEASE_CHECKLIST.md`、`docs/REVIEW_SUBMISSION.md`）。微信开发者工具当前已上传版本仍为 `1.0.8`；本批 `1.0.9` 尚未上传，没有提交审核或发布正式版。

## 可验证的仓库快照

- 工作目录：`/Users/shenjingsong/Documents/develop/lecturesApp`
- 分支：`main`
- 1.0.9 最新课程补充：HKU UG `6717 Bachelor of Social Sciences` 的 12 个 Major、687 条 Major-local 课程及 752 项测试
- 本批 HKU UG 课程补充尚未提交；当前 `HEAD` 为 `59459f8 Document latest 1.0.8 curriculum upload`
- 包版本：`1.0.9`
- `main` 与 `origin/main` 的提交同步；工作区另有本批 HKU UG source supplements、生成 catalogue/shards/分包和测试改动。
...
- 本批 HKU UG 课程补充尚未提交、推送或上传；`git diff --check` 通过。
...
2026-07-21 重新运行覆盖报告所得当前值：

- TPG：8 所学校、448 个 Programme，全部 448 个已完成来源审查，来源覆盖率 100%。
- TPG：337 个 Programme 有课程结构，Programme-local 课程共 8,896 门。
- TPG 状态：110 个 `blocked`、1 个 `archived`、0 个 `unreviewed`。
- HKU：62/62 已审查，56 个 `verified`、6 个 `blocked`、2,547 门 Programme-local 课程。
...
- UG：8 所学校、444 个 Programme、676 个 Major、13,231 条 Major-local 带代码课程、192 个 Programme 已开放课程清单；运行时 252 个 Programme 待开放，来源覆盖报告另有 253 个 Programme-level 缺口。
...
- 本地 `main` 与本地远端跟踪引用 `origin/main` 同步；本批 HKU `6717` 课程补充仍在工作区，尚未提交或推送。
...
- 当前课程补充重新运行完整检查：752/752 测试通过、`ready=true`、主包 1,817,657 bytes，所有分包低于 2 MB。
...
- 仍有 110 个 Programme 因官方公开来源缺代码、学分、Track 归属、最终审批或规则冲突而标记 `blocked`。除非出现新的官方证据，不应把它们当作待猜测补全项。
- UG 来源覆盖报告仍有 253 个缺口：112 个 index-only、23 个 reviewed-no-codes、118 个 no-source；运行时轻量目录按来源 Programme 口径显示 252 个待开放项。下一批继续从官方课程手册或部门课程页取证，不从 Programme 简介反推代码或规则。
- 多个已核验 TPG Programme 仍标记 `manual_review_required`，表示课程池可发布，但复杂路径/跨组最低要求不能由当前规则引擎自动证明。

### 发布与验收缺口

- 仓库版本资料为 `1.0.9`，微信当前已上传代码仍为 `1.0.8`；本批上传完成后也不等于已提交审核。
...
2. 1.0.9 包含 HKU `6717 Bachelor of Social Sciences` 的 12 个可核验 Major、687 条课程、生成分包及相关测试。不要把 `.playwright-cli/` 纳入提交。
...
5. 若继续 `1.0.9` 提审，先完成 iOS/Android 真机矩阵；通过后才提交审核。上传、提交审核和发布是三个独立外部动作，均需用户明确授权。
6. 若转入 UG 补数，先运行：
...
本批 HKU `6717 Bachelor of Social Sciences` 课程补充尚未提交或推送。当前工作区包含：

```text
 M docs/CODEX_HANDOFF.md
 M miniprogram/subpackages/ug-data-hku-a/ugCourseData/hku-a.js
 M miniprogram/subpackages/ug-data-hku-b/ugCourseData/hku-b.js
...
?? .playwright-cli/
?? data/ug-course-supplements/hku-6717-social-sciences-first-majors-2025.json
?? data/ug-course-supplements/hku-6717-social-sciences-remaining-majors-2025.json
```
...
npm run check:ship: PASS
  ...
  check:tpg-catalog-sync: PASS (8 schools, 448 Programmes, 8 packages, 337 course structures)
  seed/data validation: PASS
  UG supplement validation: PASS (230 supplements; 217 explicit, 13 copied)
  directory audit: PASS, blockingErrors=[]
  server --check: PASS
  Node tests: PASS, 752/752
  check:release: PASS, ready=true

git diff --check: PASS
npm run status:tpg-courses -- --school=HKU: PASS (448/448 reviewed, 0 unreviewed)
npm run status:ug-sources -- --missing-only --missing-summary --missing-limit 3: PASS (253 Programme-level source gaps: 112 index-only, 23 reviewed-no-codes, 118 no-source; runtime pending 252)
```

发布指标：

- 主包：1,817,657 bytes。
- 总估算包体：16,794,446 bytes，181 个上传文件。
- 所有 19 个 UG/TPG 分包低于 2 MB；HKBU UG 为 `ug-data-hkbu-a` 1,339,364 bytes 与 `ug-data-hkbu-b` 1,373,435 bytes；`ug-data-hku-a` 为 1,473,578 bytes，`ug-data-hku-b` 为 1,300,717 bytes。
- HKU TPG 分包：1,062,981 bytes。
- 页面：17；敏感 API：0。
...
- 微信当前已上传版本仍为 `1.0.8`；本批 `1.0.9` 尚未上传。不要把上传成功当作已提审或已发布。
