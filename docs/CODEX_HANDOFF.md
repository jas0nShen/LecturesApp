...
- 包版本：`1.0.12`
- 当前课程提交 `2831a34` 已推送到 `origin/main`；其 source supplement、生成 catalogue/shards/分包和测试均已推送。
...
- 当前 1.0.12 微信开发版仍停留在 CUHK `ENGEN English` 批次，指标为 18,827 条 UG 课程、286 个已开放 Programme 和 961/961 测试。提交 `d4e873c` 的 `JASPN` / `RELSN`、`6dedaa2` 的 `PUHSN` 与 `2831a34` 的 `TRANN` 批次均尚未上传；2026-07-28 在 `2831a34` 推送后重新检查登录状态为 `false`，连续两次新二维码登录均因未完成扫码确认返回 code 25。
...
- 当前 Git 代码完整检查：969/969 测试通过、`ready=true`、19,077 条 UG coded courses、290 个已开放 Programme、主包预估 1,842,693 bytes、总估算包体 22,393,127 bytes，所有 22 个分包低于 2 MB，敏感 API 为 0。
...
- UG 来源覆盖报告仍有 154 个缺口：36 个 index-only、25 个 reviewed-no-codes、93 个 no-source。下一批继续从官方课程手册或部门课程页取证，不从 Programme 简介反推代码或规则。
- 多个已核验 TPG Programme 仍标记 `manual_review_required`，表示课程池可发布，但复杂路径/跨组最低要求不能由当前规则引擎自动证明。

### 发布与验收缺口

- 仓库版本资料与微信开发版版本号均为 `1.0.12`，但 Git 最新 `2831a34` 所含 `JASPN` / `RELSN` / `PUHSN` / `TRANN` 批次尚未覆盖上传；上传不等于已提交审核。
...
当前 1.0.12 微信开发版对应已推送课程提交 `0cb0e93`，包含 CUHK `ENGEN English` 与 `CHPRN` / `BSCGB` 来源审查批次；Git 当前课程提交为 `2831a34`，后续 `JASPN` / `RELSN` / `PUHSN` / `TRANN` 数据尚未覆盖上传。本交接文档提交后，工作区只保留用户本地诊断目录：
...
2026-07-28 在已推送的 CUHK `TRANN Translation` 代码上重新运行：
...
npm run check:ship: PASS
  ...
  UG supplement validation: PASS (344 supplements)
  directory audit: PASS, blockingErrors=[]
  server --check: PASS
  Node tests: PASS, 969/969
  check:release: PASS, ready=true

git diff --check: PASS
npm run status:tpg-courses -- --school=HKU: PASS (448/448 reviewed, 0 unreviewed)
npm run status:ug-sources -- --missing-only --missing-summary --missing-limit 0: PASS (154 Programme-level source gaps: 36 index-only, 25 reviewed-no-codes, 93 no-source)
```

发布指标：

- 当前代码主包预估：1,842,693 bytes。
- 当前代码总估算包体：22,393,127 bytes，196 个上传文件。
