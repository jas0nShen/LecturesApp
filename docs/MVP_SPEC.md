...
7. 接入本科 Programme / Major 索引；已开放课程清单的 Major 支持加入计划、设置 Year / Term、标记已修、待安排分组和复制计划，未复核课程规则前不生成本科毕业进度；
8. 提供数据状态、数据来源、数据与隐私说明，便于用户和微信审核理解数据边界；
9. 体验版和正式版使用随包离线数据，不依赖开发环境服务。

本次 `1.0.14` 数据批次新增 CUHK Financial Technology、Biomedical Engineering 与 Computational Data Science 官方课程清单。官方来源尚未闭合逐课学分、开放选修池、替代组合或多路径完成规则，因此仅提供已核验课程的只读浏览，不生成毕业完成比例。
...
4. 持续完善 Study Plan，但不把用户排期误写为官方课程安排，也不推断未经核验的本科毕业规则；
