...
3. 需要整理待采集清单时，运行 `npm run status:ug-sources -- --school <code> --missing-only --missing-limit 10 --collector-template`，复制包含官方入口、来源状态和待补字段的任务单；也可叠加 `--readiness index-only` 或 `--readiness no-source` 生成分组任务单。
...
5. 打开报告中的 `officialUrl` 或用户提供资料，确认是否有课程代码。
6. 新增或更新 `data/ug-course-supplements/*.json`。
7. 运行 `npm run sync:ug-catalog` 生成离线数据。
8. 增加 programme count、关键 course code、search/profile 相关测试。
9. 运行 `npm run check:ship`。
10. 通过后再提交。
