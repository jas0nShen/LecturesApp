...
function writeCourseShards(catalogue) {
  ...
  fs.rmSync(COURSE_DATA_DIR, { recursive: true, force: true });
  fs.mkdirSync(COURSE_DATA_DIR, { recursive: true });

  const shardEntries = [...groupedCourses.entries()]
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([universityCode, courses]) => {
      const shardName = getCourseShardName(universityCode);
      writeCommonJsModule(path.join(COURSE_DATA_DIR, `${shardName}.js`), courses);
      return {
        universityCode,
        shardName,
