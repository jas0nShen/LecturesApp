...
// Keep sizeable university datasets comfortably below WeChat's 2 MB per
...
function splitCourseShardEntries(groupedCourses) {
  return [...groupedCourses.entries()]
    ...
    .flatMap(([universityCode, courses]) => {
      ...
      const shardCount = SHARD_COUNT_BY_UNIVERSITY[universityCode] || 1;
      if (shardCount === 1) {
        return [{ universityCode, shardName, courses }];
      }

      const chunkSize = Math.ceil(courses.length / shardCount);
      return Array.from({ length: shardCount }, (_, index) => ({
        universityCode,
        shardName: `${shardName}-${String.fromCharCode(97 + index)}`,
        courses: courses.slice(index * chunkSize, (index + 1) * chunkSize)
      }));
    });
}
...
function writeCourseShards(catalogue, options = {}) {
  const outputDir = options.outputDir || OUTPUT_DIR;
  const subpackagesDir = options.subpackagesDir || SUBPACKAGES_DIR;
  const courseShardsPath = options.courseShardsPath || path.join(outputDir, 'ugCourseShards.js');
  const groupedCourses = groupCoursesByUniversity(catalogue);
  // Remove legacy one-package roots for universities that now have multiple
  // course shards. Their public university keys remain stable in the loader.
  Object.entries(SHARD_COUNT_BY_UNIVERSITY)
    .filter(([, shardCount]) => shardCount > 1)
    .forEach(([universityCode]) => {
  ...
  const packageNames = Object.fromEntries([...entriesByUniversity.entries()].map(([universityCode, entries]) => [
    universityCode,
    // wx.loadSubPackage expects the configured root path rather than the
    // display label used by release reports.
    entries.map((entry) => `subpackages/ug-data-${entry.shardName}`)
  ]));
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(courseShardsPath, [
    '// Generated by npm run sync:ug-catalog. Do not edit manually.',
    'const nodeLoaders = {',
    ...nodeLoaderLines,
    '};',
...
function writeCatalogue(catalogue, options = {}) {
  const outputDir = options.outputDir || OUTPUT_DIR;
  const outputPath = options.outputPath || path.join(outputDir, 'ugCatalogue.js');
  const { courseShardCounts, codedCourseCount } = writeCourseShards(catalogue, options);
...
function listFiles(directory) {
  ...
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    return entry.isDirectory()
      ? listFiles(fullPath).map((child) => path.join(entry.name, child))
      : [entry.name];
  });
}

function normalizeGeneratedContent(filePath, content) {
  if (path.basename(filePath) !== 'ugCatalogue.js') return content;
  return content.replace(/"generatedAt":\s*"[^"]+"/g, '"generatedAt":"<generated>"');
}

function assertGeneratedOutputMatches(tempRoot) {
  const compareFile = (actualPath, expectedPath, label) => {
    if (!fs.existsSync(actualPath) || !fs.existsSync(expectedPath)) {
      throw new Error(`UG generated output is out of sync: ${label}`);
    }
    const actual = normalizeGeneratedContent(actualPath, fs.readFileSync(actualPath, 'utf8'));
    const expected = normalizeGeneratedContent(expectedPath, fs.readFileSync(expectedPath, 'utf8'));
    if (actual !== expected) throw new Error(`UG generated output is out of sync: ${label}`);
  };
  compareFile(OUTPUT_PATH, path.join(tempRoot, 'utils', 'ugCatalogue.js'), 'utils/ugCatalogue.js');
  compareFile(COURSE_SHARDS_PATH, path.join(tempRoot, 'utils', 'ugCourseShards.js'), 'utils/ugCourseShards.js');

  const actualRoot = SUBPACKAGES_DIR;
  const expectedRoot = path.join(tempRoot, 'subpackages');
  const actualFiles = listFiles(actualRoot).filter((file) => file.startsWith('ug-data-'));
  const expectedFiles = listFiles(expectedRoot);
  const relativeFiles = [...new Set(actualFiles.concat(expectedFiles))].sort();
  relativeFiles.forEach((file) => {
    const actualPath = path.join(actualRoot, file);
    const expectedPath = path.join(expectedRoot, file);
    if (!fs.existsSync(actualPath) || !fs.existsSync(expectedPath)) {
      throw new Error(`UG generated output is out of sync: subpackages/${file}`);
    }
    const actual = fs.readFileSync(actualPath, 'utf8');
    const expected = fs.readFileSync(expectedPath, 'utf8');
    if (actual !== expected) throw new Error(`UG generated output is out of sync: subpackages/${file}`);
  });
}

function main(argv = process.argv.slice(2)) {
  const sourceDirIndex = argv.indexOf('--source-dir');
  const sourceDir = sourceDirIndex === -1 ? DEFAULT_SOURCE_DIR : path.resolve(argv[sourceDirIndex + 1]);
  const checkOnly = argv.includes('--check');
  const catalogue = buildCatalogue(sourceDir);
  if (checkOnly) {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ug-catalog-check-'));
    try {
      writeCatalogue(catalogue, {
        outputDir: path.join(tempRoot, 'utils'),
        outputPath: path.join(tempRoot, 'utils', 'ugCatalogue.js'),
        courseShardsPath: path.join(tempRoot, 'utils', 'ugCourseShards.js'),
        subpackagesDir: path.join(tempRoot, 'subpackages')
      });
      assertGeneratedOutputMatches(tempRoot);
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
    console.log('UG generated catalogue is in sync.');
    return;
  }
  writeCatalogue(catalogue);
