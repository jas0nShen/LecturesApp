...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6591);
  assert.equal(summary.totals.programmeWithCoursesCount, 86);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3250');
  ...
  assert.match(template, /1\. POLYU · JS3250 · Bachelor of Arts \(Honours\) Scheme in Applied Social Sciences/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 184);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3250');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：184 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3250-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3250-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3250 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3250"/);
