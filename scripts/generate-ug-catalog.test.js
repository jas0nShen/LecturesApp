...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6200);
  assert.equal(summary.totals.programmeWithCoursesCount, 80);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3223');
  ...
  assert.match(template, /1\. POLYU · JS3223 · Bachelor of Science \(Honours\) Scheme in Data Science and Artificial Intelligence/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 190);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3223');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：190 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3223-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3223-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3223 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3223"/);
