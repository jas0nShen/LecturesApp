...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 5389);
  assert.equal(summary.totals.programmeWithCoursesCount, 74);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3130');
  ...
  assert.match(template, /1\. POLYU · JS3130 · Bachelor of Science \(Honours\) Scheme in Spatial Data Science and Smart Cities/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 199);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3130');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：199 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3130-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3130-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3130 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3130"/);
