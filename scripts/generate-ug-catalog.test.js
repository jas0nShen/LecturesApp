...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 5723);
  assert.equal(summary.totals.programmeWithCoursesCount, 78);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3180');
  ...
  assert.match(template, /1\. POLYU · JS3180 · Bachelor of Engineering \(Honours\) \/ Bachelor of Science \(Honours\) Scheme in Information and Artificial Intelligence Engineering/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 194);
  assert.equal(groups.reviewedNoCourseCodes.length, 2);
  assert.equal(groups.noSource.length, 171);
  assert.equal(groups.sourceIndexOnly[0].schoolCode, 'POLYU');
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3180');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：194 个/);
  assert.match(plan, /D\. 已核实官网暂无公开课程码：2 个/);
...
test('UG source coverage report can filter reviewed no-code programmes', () => {
  ...
  assert.equal(polyu.filteredMissingProgrammeCount, 2);
  assert.equal(polyu.missingProgrammes.length, 2);
  assert.equal(polyu.missingProgrammes[0].code, 'JS3011');
  assert.equal(polyu.missingProgrammes[0].sourceReviewStatus, 'no_public_course_codes');
  assert.equal(polyu.missingProgrammes[1].code, 'JS3160');
  ...
  assert.match(template, /当前筛选：reviewed-no-codes · 2 个/);
});
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3180-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3180-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3180 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3180"/);
