...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6841);
  assert.equal(summary.totals.programmeWithCoursesCount, 92);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3739');
  ...
  assert.match(template, /1\. POLYU · JS3739 · Bachelor of Engineering \(Honours\) in Civil Engineering/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 171);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3739');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：171 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3739-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3739-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3739 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3739"/);
