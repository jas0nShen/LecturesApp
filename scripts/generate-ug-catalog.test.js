...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6945);
  assert.equal(summary.totals.programmeWithCoursesCount, 96);
...
test('UG source coverage report can filter missing programmes by source readiness', () => {
  ...
  assert.equal(hku.missingProgrammeCount, 115);
  ...
  assert.match(template, /待补 Programme：115/);
...
test('UG source coverage report can generate a collector template for missing programmes', () => {
  ...
  assert.match(template, /待补 Programme：115/);
  assert.match(template, /来源状态：0 source importable · 0 coded not importable · 104 index only · 1 reviewed no course codes · 10 no source/);
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 158);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, '6080');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：158 个/);
