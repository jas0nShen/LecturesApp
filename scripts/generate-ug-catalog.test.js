...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3478');
  ...
  assert.match(template, /1\. POLYU · JS3478 · Bachelor of Science \(Honours\) in Medical Laboratory Science/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 177);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3478');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：177 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3478-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3478-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3478 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3478"/);
