...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3006');
  ...
  assert.match(template, /1\. POLYU · JS3006 · Bachelor’s Degree Scheme in Computer and Mathematical Sciences/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 163);
  assert.equal(groups.reviewedNoCourseCodes.length, 16);
  assert.equal(groups.noSource.length, 171);
  assert.equal(groups.sourceIndexOnly[0].schoolCode, 'POLYU');
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3006');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：163 个/);
  assert.match(plan, /D\. 已核实官网暂无公开课程码：16 个/);
...
test('UG source coverage report can filter reviewed no-code programmes', () => {
  ...
  assert.equal(polyu.filteredMissingProgrammeCount, 16);
  ...
  assert.match(template, /当前筛选：reviewed-no-codes · 16 个/);
});
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3006-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3006-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3006 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3006"/);
