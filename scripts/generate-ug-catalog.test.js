...
test('CityU Energy and Environment exposes verified common first-year courses', () => {
  const catalogue = require('../miniprogram/utils/ugCatalogue');
  const programme = catalogue.programmes.find((item) => item.jupasCode === 'JS1051');
  ...
  majors.forEach((major) => {
    const courseCodes = catalogue.courses
      .filter((course) => course.majorId === major.id)
      .map((course) => course.courseCode);
    assert(courseCodes.includes('SEE1002'));
    assert(courseCodes.includes('SEE1003'));
    assert(courseCodes.includes('CHEM1300'));
  });
});
...
test('CityU law and accountancy double degree exposes its verified first-year study plan', () => {
  const catalogue = require('../miniprogram/utils/ugCatalogue');
  const programme = catalogue.programmes.find((item) => item.jupasCode === 'JS1062');
  const major = catalogue.majors.find((item) => item.programmeId === programme.id);
  ...
  assert(courseCodes.includes('LW3605A'));
  assert(courseCodes.includes('GE2262'));
...
test('CUHK Philosophy exposes the verified required and capstone courses', () => {
  const catalogue = require('../miniprogram/utils/ugCatalogue');
  const programme = catalogue.programmes.find((item) => item.universityCode === 'CUHK' && item.jupasCode === 'JS4094');
  const major = catalogue.majors.find((item) => item.programmeId === programme.id);
  const courses = catalogue.courses.filter((item) => item.majorId === major.id);

  assert.equal(courses.length, 10);
  assert(courses.some((item) => item.courseCode === 'PHIL1310'));
  assert(courses.some((item) => item.courseCode === 'PHIL2020'));
  assert(courses.some((item) => item.courseCode === 'PHIL2050'));
  assert(courses.some((item) => item.courseCode === 'PHIL4800' && item.courseType === 'capstone'));
});
...
test('large university course rows are split into evenly sized generated subpackage shards', () => {
  ['CITYU', 'CUHK', 'HKBU', 'HKUST', 'POLYU'].forEach((universityCode) => {
