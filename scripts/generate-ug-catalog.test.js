...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6794);
  assert.equal(summary.totals.programmeWithCoursesCount, 90);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3375');
  ...
  assert.match(template, /1\. POLYU · JS3375 · Bachelor of Engineering \(Honours\) in Environmental Engineering and Sustainable Development/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 178);
  assert.equal(groups.reviewedNoCourseCodes.length, 6);
  assert.equal(groups.noSource.length, 171);
  assert.equal(groups.sourceIndexOnly[0].schoolCode, 'POLYU');
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3375');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：178 个/);
  assert.match(plan, /D\. 已核实官网暂无公开课程码：6 个/);
...
test('UG source coverage report can filter reviewed no-code programmes', () => {
  ...
  assert.equal(polyu.filteredMissingProgrammeCount, 6);
  assert.equal(polyu.missingProgrammes.length, 6);
  ...
  assert.match(template, /当前筛选：reviewed-no-codes · 6 个/);
});
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3375-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3375-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3375 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3375"/);
