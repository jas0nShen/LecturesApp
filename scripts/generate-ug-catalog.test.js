...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 5649);
  assert.equal(summary.totals.programmeWithCoursesCount, 77);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3170');
  ...
  assert.match(template, /1\. POLYU · JS3170 · Bachelor of Engineering \(Honours\) Scheme in Electrical Engineering/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 195);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3170');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：195 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3170-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3170-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3170 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3170"/);
