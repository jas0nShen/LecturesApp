...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6425);
  assert.equal(summary.totals.programmeWithCoursesCount, 83);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3240');
  ...
  assert.match(template, /1\. POLYU · JS3240 · Bachelor of Arts \(Honours\) in English and Applied Linguistics/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 187);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3240');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：187 个/);
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3240-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3240-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3240 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3240"/);
