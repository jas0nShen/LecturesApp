...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 7193);
  assert.equal(summary.totals.programmeWithCoursesCount, 102);
...
test('UG source coverage report can filter missing programmes by source readiness', () => {
  ...
  assert.equal(hku.missingProgrammeCount, 109);
  ...
  assert.match(template, /待补 Programme：109/);
...
test('UG source coverage report can generate a collector template for missing programmes', () => {
  ...
  assert.match(template, /待补 Programme：109/);
  assert.match(template, /来源状态：0 source importable · 0 coded not importable · 97 index only · 2 reviewed no course codes · 10 no source/);
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 151);
  ...
  assert.equal(groups.sourceIndexOnly[0].code, '6248');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：151 个/);
