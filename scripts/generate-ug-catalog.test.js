...
test('UG source coverage report includes generated catalogue supplement coverage', () => {
  ...
  assert.equal(summary.totals.codedCourseCount, 6922);
  assert.equal(summary.totals.programmeWithCoursesCount, 95);
...
test('UG source coverage report can prioritize launch data collection batches', () => {
  ...
  assert.equal(missing[0].code, 'JS3004');
  ...
  assert.match(template, /1\. POLYU · JS3004 · Bachelor’s Degree Scheme in Construction and Environment/);
});
...
test('UG source coverage report can build a grouped missing data batch plan', () => {
  ...
  assert.equal(groups.sourceIndexOnly.length, 165);
  assert.equal(groups.reviewedNoCourseCodes.length, 14);
  assert.equal(groups.noSource.length, 171);
  assert.equal(groups.sourceIndexOnly[0].schoolCode, 'POLYU');
  assert.equal(groups.sourceIndexOnly[0].code, 'JS3004');
  ...
  assert.match(plan, /C\. 需打开官方入口核实课程码：165 个/);
  assert.match(plan, /D\. 已核实官网暂无公开课程码：14 个/);
...
test('UG source coverage report can filter reviewed no-code programmes', () => {
  ...
  assert.equal(polyu.filteredMissingProgrammeCount, 14);
  ...
  assert.match(template, /当前筛选：reviewed-no-codes · 14 个/);
});
...
test('UG source coverage report can generate a safe supplement starter template', () => {
  ...
  assert.equal(buildSupplementFileName(missing), 'polyu-js3004-courses-2026.json');
  assert.match(template, /建议文件：data\/ug-course-supplements\/polyu-js3004-courses-2026\.json/);
  assert.match(template, /"provider": "POLYU JS3004 undergraduate course supplement"/);
  assert.match(template, /"jupasCode": "JS3004"/);
