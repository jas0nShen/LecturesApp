...
test('current mini-program passes automated release readiness checks', () => {
  ...
  assert.equal(result.release.version, '1.0.3');
  ...
  assert.equal(result.metrics.tpgProgrammeWithCoursesCount, 253);
  assert.equal(result.metrics.tpgCourseCount, 6125);
...
test('WeChat review version description stays within the 200 character limit', () => {
  ...
  assert(versionDescription.includes('1.0.3'));
