...
test('current mini-program passes automated release readiness checks', () => {
  ...
  assert.equal(result.release.version, '1.0.16');
  ...
  assert.equal(result.metrics.ugCodedCourseCount, 20805);
  assert.equal(result.metrics.ugProgrammeWithCoursesCount, 315);
...
test('WeChat review version description stays within the 200 character limit', () => {
  ...
  assert(versionDescription.includes('1.0.16'));
