...
test('current mini-program passes automated release readiness checks', () => {
  ...
  assert.equal(result.release.version, '1.0.15');
  ...
  assert.equal(result.metrics.ugMajorCount, 686);
  assert.equal(result.metrics.ugCodedCourseCount, 20674);
  assert.equal(result.metrics.ugProgrammeWithCoursesCount, 313);
  ...
  assert.equal(result.metrics.subpackageBytes.length, 23);
  assert(result.metrics.subpackageBytes.some((subpackage) => subpackage.root === 'subpackages/ug-data-cuhk-a'));
  assert(result.metrics.subpackageBytes.some((subpackage) => subpackage.root === 'subpackages/ug-data-cuhk-b'));
...
test('WeChat review version description stays within the 200 character limit', () => {
  ...
  assert(versionDescription.includes('1.0.15'));
