...
test('current mini-program passes automated release readiness checks', () => {
  ...
  assert.equal(result.release.version, '1.0.13');
  ...
  assert.equal(result.metrics.ugCodedCourseCount, 19584);
  assert.equal(result.metrics.ugProgrammeWithCoursesCount, 299);
...
test('WeChat review version description stays within the 200 character limit', () => {
  ...
  assert(versionDescription.includes('1.0.13'));
